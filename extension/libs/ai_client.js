// libs/ai_client.js
// AI API 호출을 담당하는 범용 클라이언트 (OpenAI, Google Gemini 지원)

console.log("[ai_client] loaded");

async function _callOpenAI(apiKey, model, messages) {
  if (!apiKey) {
    throw new Error("API Key가 설정되지 않았습니다. 옵션 페이지에서 설정해주세요.");
  }
...
async function _callGoogleGemini(apiKey, model, messages) {
  if (!apiKey) {
    throw new Error("API Key가 설정되지 않았습니다. 옵션 페이지에서 설정해주세요.");
  }

  // Gemini API는 messages 형식이 약간 다름. role: 'user'/'model'
  const contents = messages.map(msg => ({
    role: msg.role === 'system' ? 'user' : msg.role, // system -> user
...
async function summarizeBookmarks(bookmarks, options = {}) {
  ...
  const { api_provider, api_key } = await new Promise((resolve) =>
    chrome.storage.sync.get(["api_provider", "api_key"], resolve)
  );
  ...
  if (api_provider === 'google') {
    return _callGoogleGemini(api_key, model, messages);
  }
  // 기본값은 OpenAI
  return _callOpenAI(api_key, model, messages);
}
...
async function groupBookmarks(bookmarks, options = {}) {
  ...
  let rawResult;
  if (api_provider === 'google') {
    rawResult = await _callGoogleGemini(api_key, model, messages);
  } else {
    rawResult = await _callOpenAI(api_key, model, messages);
  }
