...
─────────────────────────────────────────────────────────────────
           [사용자]  
            ...
            ▼
   ┌──────────────────────────┐
   │   Chrome Extension       │ ← 북마크 트리 / 요약 카드 / 관리
   │  (User Interface Layer)  │
   └──────────────────────────┘
            │ 1) ingest/summarize/search
            ▼
──────────────── API Gateway (FastAPI) ─────────────────────────
            ▲
            │ 2) OpenAI API 활용 (요약/분류)
            ▼
    [OpenAI API]  ← fallback 모델 포함
─────────────────────────────────────────────────────────────────
            ▲
            │ 3) MCP 기반 모델러/검색/자동화
            ▼
        [MCP Server] ← 북마크 분석/벡터 검색/자동 작업
─────────────────────────────────────────────────────────────────
            ▲
            │ 4) Tracking Layer
            ▼
      [Event Tracking DB]  (DuckDB → BigQuery/ClickHouse 확장)
─────────────────────────────────────────────────────────────────
```
